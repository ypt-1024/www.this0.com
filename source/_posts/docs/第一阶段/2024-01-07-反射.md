...
方案1：`在编译和运行时都完全知道类型的具体信息`，在这种情况下，我们可以直接先使用instanceof运算符进行判断，再利用`强制类型转换符将其转换成运行时类型的变量即可`。

方案2：编译时根本`无法预知该对象和类的真实信息`，程序`只能依靠反射`来发现该对象和类的真实信息，这就必须使用反射。获取运行时信息
...
![image-20220524101441695.png](https://blog-resources.this0.com/image/202403301627088.png?x-oss-process=style/this0-blog)
...
### 2.反射机制-反射、Class 的理解与获取 Class 实例的方式
...
以上的方法返回值的类型是一个Class类，此类是Java反射的源头，实际上`所谓反射从程序的运行结果来看也很好理解，即：可以通过对象反射求出类的名称。`
...
![image-20220514180100176.png](https://blog-resources.this0.com/image/202403301627007.png?x-oss-process=style/this0-blog)
...
```java
Class clazz = "www.this0.com".getClass();
```
...
public class GetClassObject {
    ...
    public void test01() throws ClassNotFoundException{
        ...
        Class c3 = Class.forName("com.this0.classtype.GetClassObject");
        Class c4 = ClassLoader.getSystemClassLoader().loadClass("com.this0.classtype.GetClassObject");
...
###  3.反射机制-类的加载过程与类加载器的理解

#### 1. 类的加载与ClassLoader的理解

##### 1.1 `类的生命周期`

类在内存中完整的生命周期：`加载-->链接-->初始化-->使用-->卸载`。其中`链接过程又分为：验证、准备、解析三个阶段。`

![image-20220417173459849.png](https://blog-resources.this0.com/image/202403301627060.png?x-oss-process=style/this0-blog )

`概要：`

1. **加载（Loading）：** 将类的字节码文件加载到内存中。这通常由类加载器执行，并将字节码存储在方法区。
2. **验证（Verification）：** 确保被加载的类的字节码符合Java虚拟机规范，以防止安全问题和运行时错误。
3. **准备（Preparation）：** 为类的静态变量分配内存，并设置默认初始值。这一阶段发生在类加载后，但在初始化之前。
4. **解析（Resolution）：** 将类、接口、字段和方法的符号引用解析为直接引用，使得各个类之间能够建立联系。
5. **初始化（Initialization）：** 对类进行初始化，包括执行静态变量赋值和执行静态代码块。这是类加载的最后一个阶段。
6. **使用（Usage）：** 类被应用程序使用，其实例被创建，方法被调用等。
7. **卸载（Unloading）：** 在某些情况下，当一个类不再被引用，且没有任何活动的实例时，Java虚拟机可能会卸载该类，释放相应的内存。

#### 2. 类的加载过程

当程序主动使用某个类时，如果该类还未被加载到内存中，系统会通过`装载、链接、初始化`三个步骤来对该类进行初始化。如果没有意外，JVM将会连续完成这三个步骤，所以有时也把`这三个步骤统称为类加载`。

![image-20220417171411631.png](https://blog-resources.this0.com/image/202403301627048.png?x-oss-process=style/this0-blog)
...
![image-20220417173647473.png](https://blog-resources.this0.com/image/202403301627018.png?x-oss-process=style/this0-blog )
...
![image-20220417173616344.png](https://blog-resources.this0.com/image/202403301627065.png?x-oss-process=style/this0-blog )
...
![image-20220417173819953.png](https://blog-resources.this0.com/image/202403301627620.png?x-oss-process=style/this0-blog )
...
![image-20220417174032702.png](https://blog-resources.this0.com/image/202403301627993.png?x-oss-process=style/this0-blog )
...
###  4.反射机制-反射的应用：创建运行时类的对象、获取运行时类的完整结构
...
//        this0Class obj = new this0Class();//编译期间无法创建

        Class<?> clazz = Class.forName("com.this0.ext.demo.this0Class");
        //clazz代表com.this0.ext.demo.this0Class类型
        //clazz.newInstance()创建的就是this0Class的对象
        Object obj = clazz.newInstance();
    ...
    public void test3()throws Exception{
        //(1)获取Class对象
        Class<?> clazz = Class.forName("com.this0.ext.demo.this0Demo");
        /*
         * 获取this0Demo类型中的有参构造
         * 如果构造器有多个，我们通常是根据形参【类型】列表来获取指定的一个构造器的
         * 例如：public this0Demo(String title, int num)
         */
        //(2)获取构造器对象
...
public class OtherTest {
    ...
    public void test2(){
        ...
        System.out.println(superclass);//class com.this0.java1.Creature
    }
    /*
    	获取运行时类的所在的包
     */
    ...
    public void test6(){
        ...
        System.out.println(genericSuperclass);//com.this0.java1.Creature<java.lang.String>
    }
}
```
...
public class TestField {
    public static void main(String[] args)throws Exception {
        //1、获取Student的Class对象
        Class clazz = Class.forName("com.this0.reflect.Student");
...
##### 1.2 调用指定的方法![image-20220417181700813.png](https://blog-resources.this0.com/image/202403301627666.png?x-oss-process=style/this0-blog )
...
> 之前的章节简单提过，当时没有学反射，知识水平不够理解自定义注解，到这里才应该理解

```java
package com.this0.annotation;
...
public class ReflectionTest {
    ...
    @Test
    public void test1() throws Exception {
        String className = "com.this0.java1.Person";
...
public class ReflectionTest {
    ...
    public void test2() throws Exception {
        String info = (String) invoke("com.this0.java1.Person", "show");
