...
说明：`线程作为调度和执行的单位，每个线程拥独立的运行栈和程序计数器(pc)`，线程切换的开销小。
  ![image.png](https://blog-resources.this0.com/image/202403301620512.png?x-oss-process=style/this0-blog )
`回顾内存结构：`
![image.png](https://blog-resources.this0.com/image/202403301620777.png?x-oss-process=style/this0-blog )
进程可以细化为多个线程。
每个线程，拥有自己`独立的栈、程序计数器`
...
1. 实现的方式没类的单继承性的局限性(无法继承其他的)
  ...
  2. 实现的方式更适合来处理`多个线程共享数据`的情况`（接口实现类天然能共享对象,比如适合用this当锁）`。
...
- notify():一旦执行此方法，就会`唤醒被wait的一个线程。如果有多个线程被wait，就唤醒优先级高的那个，线程优先级的作用就出来了。`
...
​    说明：高优先级的线程要抢占低优先级线程cpu的执行权。但是`只是从概率上讲，高优先级的线程高概率的情况下被执行。并不意味着只有当高优先级的线程执行完以后，低优先级的线程才执行。`
...
`阻塞又可以分为3种，无限等待（waiting），有限等待（time waiting），锁阻塞`
